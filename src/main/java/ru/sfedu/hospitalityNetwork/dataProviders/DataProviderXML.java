...
public class DataProviderXML implements DataProvider {
    ...
    public void initDB() {
        try {
            ...
            insertIntoXML(OfferHost.class, new ArrayList<>(), true);
            insertIntoXML(OfferHost.class, new ArrayList<>(), true);
            insertIntoXML(Comment.class, new ArrayList<>(), true);
            insertIntoXML(Response.class, new ArrayList<>(), true);
        } catch (IOException e) {
            log.error(e);
        }
    }
    ...
    public Outcome editOfferHost(@NotNull String offerId,
        ...
        if (offerHostOptional.isPresent()) {
            ...
            return rewriteOfferHost(offerHosts);
        } else {
            log.error(Constants.OFFER_NOT_FOUND);
            return Outcome.FAILED;
        }
    ...
    private Optional<OfferGuest> createOfferGuestOptional(
        ...
        try {
            ...
            if (user.isPresent() && user.get().getOffer() == null) {
                String uuid = UUID.randomUUID().toString();
    ...
    private Optional<List<OfferGuest>> getOfferGuestsOptional() {
        try {
            return Optional.of(getFromXML(OfferGuest.class));
        } catch (IOException e) {
            log.error(e);
            return Optional.empty();
        }
    ...
    private Outcome rewriteOfferGuests(List<OfferGuest> offerGuests) {
        try {
            insertIntoXML(OfferGuest.class, offerGuests, true);
            return Outcome.SUCCESS;
        } catch (IOException e) {
            log.error(e);
            return Outcome.FAILED;
    ...
    private Optional<List<OfferHost>> getOfferHostsOptional() {
        try {
            return Optional.of(getFromXML(OfferHost.class));
        } catch (IOException e) {
            log.error(e);
            return Optional.empty();
        }
    ...
    private Outcome rewriteOfferHost(List<OfferHost> offerHosts) {
        try {
            insertIntoXML(OfferHost.class, offerHosts, true);
            return Outcome.SUCCESS;
        } catch (IOException e) {
            log.error(e);
            return Outcome.FAILED;
    ...
    private Optional<List<Comment>> getCommentsOptional() {
        try {
            return Optional.of(getFromXML(Comment.class));
        } catch (IOException e) {
            log.error(e);
            return Optional.empty();
        }
    ...
    private Outcome rewriteComments(List<Comment> comments) {
        try {
            insertIntoXML(Comment.class, comments, true);
            return Outcome.SUCCESS;
        } catch (IOException e) {
            log.error(e);
            return Outcome.FAILED;
    ...
    private Outcome addResponseToOffer(@NotNull Response response,
        ...
        if (optionalOfferHost.isPresent()) {
            Optional<User> userOptional = getUserOptional(optionalOfferHost.get().getIdUser());
            ...
            userOptional.get().setOffer(optionalOfferHost.get());
            Optional<List<User>> optionalUserList = getUsersOptional();
            List<User> users = optionalUserList.get();
            users = users.stream()
                    .filter(user -> !user.getIdUser()
                            .equals(optionalOfferHost.get().getIdUser()))
                    .collect(Collectors.toList());
            users.add(userOptional.get());

            rewriteUsers(users);
            rewriteOfferHost(offerHosts);
            return Outcome.SUCCESS;
            ...
            userOptional.get().setOffer(optionalOfferGuest.get());
            Optional<List<User>> optionalUserList = getUsersOptional();
            List<User> users = optionalUserList.get();
            users = users.stream()
                    .filter(user -> !user.getIdUser()
                            .equals(optionalOfferGuest.get().getIdUser()))
                    .collect(Collectors.toList());
            users.add(userOptional.get());
    ...
    private Optional<List<Response>> getResponsesOptional() {
        try {
            return Optional.of(getFromXML(Response.class));
        } catch (IOException e) {
            log.error(e);
            return Optional.empty();
        }
    ...
    private Outcome rewriteResponse(List<Response> responses) {
        try {
            insertIntoXML(Response.class, responses, true);
            return Outcome.SUCCESS;
        } catch (IOException e) {
            log.error(e);
            return Outcome.FAILED;
    ...
    private Outcome deleteResponseFromOffer(String offerId, String responseId) {
        ...
        if (optionalOfferHost.isPresent() && optionalResponsesOffer.isPresent()) {
            ...
            Optional<User> optionalUser = getUserOptional(optionalOfferHost.get().getIdUser());
            ...
            optionalUser.get().setOffer(optionalOfferHost.get());
            Optional<List<User>> optionalUserList = getUsersOptional();
            List<User> users = optionalUserList.get();
            users = users.stream()
                    .filter(user -> !user.getIdUser()
            ...
            Optional<User> optionalUser = getUserOptional(optionalOfferGuest.get().getIdUser());
            ...
            optionalUser.get().setOffer(optionalOfferGuest.get());
            Optional<List<User>> optionalUserList = getUsersOptional();
            List<User> users = optionalUserList.get();
            users = users.stream()
                    .filter(user -> !user.getIdUser()
